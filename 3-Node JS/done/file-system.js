...
// --------------------
try {
  fs.appendFileSync('./txt/final.txt', '\nUpdated content');
...
// Read File (Async)
// --------------------
fs.readFile('./txt/newfile.txt', 'utf8', (err, data) => {
...
// Write File (Async)
// --------------------
fs.writeFile('./txt/newfile.txt', 'Hello, World!', err => {
...
fs.readFile('start.txt', 'utf-8', (err, data1) => {
  ...
  fs.readFile(`${data1}.txt`, 'utf-8', (err, data2) => {
    ...
    fs.readFile('append.txt', 'utf-8', (err, data3) => {
      ...
      fs.writeFile('final.txt', `${data2} ${data3}`, err => {
        if (err) return console.error(err);
        console.log('File written successfully!');
      });
    });
...
const filePath = path.join(__dirname, 'txt', 'input.txt');
...
fsPromises.readFile('start.txt', 'utf-8')
  .then(data1 => fsPromises.readFile(`${data1}.txt`, 'utf-8'))
  .then(data2 =>
    fsPromises.readFile('append.txt', 'utf-8')
      .then(data3 => fsPromises.writeFile('final.txt', `${data2} ${data3}`))
  )
...
async function readFiles() {
  try {
    const data1 = await fsPromises.readFile('start.txt', 'utf-8');
    const data2 = await fsPromises.readFile(`${data1}.txt`, 'utf-8');
    const data3 = await fsPromises.readFile('append.txt', 'utf-8');

    await fsPromises.writeFile('final.txt', `${data2} ${data3}`);
    console.log('File written successfully!');
  } catch (err) {
